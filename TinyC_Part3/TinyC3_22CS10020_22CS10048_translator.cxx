...
void SymbolTable::print() { 
    ...
    for (auto &sym : this->symbols) {
        ...
        cout << setw(35) << sym.second.type->toString();
        cout << setw(20);

        switch (sym.second.category) {
            case Symbol::LOCAL:
                cout << "local";
                break;
            case Symbol::GLOBAL:
        ...
        // Store nested tables to be printed later
        if (sym.second.nestedTable) {
            childTables.push_back(sym.second.nestedTable);
        }
    }
